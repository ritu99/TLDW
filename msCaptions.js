...
function talkToMicrosoft(url){
	...
	params = {"name" : "1234", "privacy" : "Private", "videoUrl":url}
	request.post({
		headers: headers,
...
app.post("/summarize_video", (req, res) =>
	{
		url = "https://" + "helloacm.com" + "/api/video/?cached&lang=en&page=youtube&hash=" + hash + "&video="+encodeURIComponent(_url)
		k = request.get(url,
			function(error, response, body){
				talkToMicrosoft(body['url'])
				console.log(body);
			});
	}
);
